...
def main():
    ...
    start_ix = 7*128 # index of the first file
    str_from_to = "_"+str(start_ix)+"to"+str(start_ix+from_to)+"_"
    file1 = open("mc4_"+str_from_to+"stats.txt","w") #write mode
    dict_count = {}
    avg_dict_count = {}
    total_samples = 0
    total_filesize = 0
    ...
    formatted_dataset = open("/kuacc/users/merdogan18/hpc_run/tdd-plms/data_statistics/Processed-MC4/xmc4_"+str_from_to+"data_formatted.json", "a")
    count_dictionaries = open("/kuacc/users/merdogan18/hpc_run/tdd-plms/data_statistics/Processed-MC4/mc4_"+str_from_to+"stats_formatted.json", "a")
    
    for doc_num in range(start_ix, start_ix+from_to):
        print(doc_num)
        dict_count[str(doc_num)] = {}
        avg_dict_count[str(doc_num)] = {}
        num_str = "0"*(5-len(str(doc_num)))+str(doc_num)
        doc_name = 'c4-tr.tfrecord-'+num_str+'-of-01024.json'
        dir_name = "/datasets/mc4_tr/"+doc_name
        ...
        for i in range(len(data_ins)): #range(len(data_ins)):
            ...
            dict_sample = {}
            dict_sample["source"] = data_ins[i]['url']
            dict_sample["doc_id"] = ct_doc
            dict_sample["language"] = "tr"
            dict_sample["corpus"] = "MC-4" + str_from_to
    ...
    with open("/kuacc/users/merdogan18/hpc_run/tdd-plms/data_statistics/Processed-MC4/summary_mc4_"+str_from_to+"stats_formatted.json", "w") as outfile:
